...
static void putText_impl(HDC dc, HFONT f, COLORREF c, const wchar_t *s, float x, float y, int align, int src)
{
    ...
    uiTraceText(dc, f, c, s, (float)rc.left, (float)rc.top, (float)sz.cx, (float)sz.cy, src);
    SetTextColor(dc, c);
    DrawTextW(dc, s, -1, &rc, DT_LEFT | DT_TOP | DT_NOCLIP);
    uiMark(dc, (float)rc.left, (float)rc.top, (float)sz.cx, (float)sz.cy, src);
}
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_KEYDOWN:
        ...
        else if (wp == VK_F1)  { gState = ST_PLAY;     gDumpTag = 1;  gDumpLeft = 60; }
