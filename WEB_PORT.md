...
## 6.7 ★ 线上可用：从"画面全黑"到"文字正常"的两次根因

第六节之后又踩了两个坑，都属**"不报错、只是画错"**这一类，
而且都只在真浏览器里才暴露。记在这里是因为它们各自代表一个反复出现的模式。

### 6.7.1 「素材载入 0 张、画面全黑」—— wsprintfW 的 `%s` 语义

现象：线上能加载、WASM 能启动、引导页正常走完、**0 个 JS 异常、0 个失败请求**，
但世界层像素全是 0，画面只有黑底。诊断 wasm 侧状态：
`gameArtCount() == 0`（应为 853）。

先排除了"资源没打包"：MEMFS 里 864 个条目都在，`/assets/rgplant_00.png`
有 64887 字节、读得到。所以问题在**路径字符串**，不在文件。

根因：`wsprintfW` 第一版实现成 `vswprintf(out, 512, fmt, ap)`，而
宽格式串里的 `%s`：
- **MSVC 语义**（本机 MinGW）收 `wchar_t*`
- **POSIX 语义**（emcc 的 musl）收 `char*`，宽串必须写 `%ls`

引擎里 38 处 `wsprintfW(path, L"%s%s.png", gAssetDir, name)` 传的都是
`wchar_t*` —— 本机正确，musl 上被当 `char*` 读 → 拼出垃圾路径 →
`_wfopen` 全部失败 → 853 张素材一张没加载，且**不报任何错**。

修法：不"顺手把那 38 处改成 `%ls`"（它们在 Windows 上本来就是对的），
而是**自己解析格式串、按 Windows 语义实现** `wsprintfW`。
额外好处是函数变成平台无关的，本机测试才真的覆盖线上代码。
测试见 `_test_web_printf.c`（39 条用例，含 `%03d` 负数补位、`%u` 无符号
两个它自己抓出来的真 bug）。

### 6.7.2 「整屏一个字都没有」—— n = -1 被当成"0 个字符"

素材修好后画面正常了，但**一个字的都没有**：没有阳光数、没有卡价、
没有波次、没有标题。文档 §2.3 早写好了"Canvas2D 光栅 → 缓存位图"的方案，
但**从未实现**（`platWebGlyph` 有声明有实现，外壳里没有一个调用点）。

把它补完时，又查出底层一个一直被掩盖的 bug：

```c
putText():  DrawTextW(dc, s, -1, &rc, ...)      /* pvz.c:394 */
```
Win32 里 `n = -1` 表示"按 NUL 结尾"，而 `drawTextCore` 直接拿它当计数：
`for (i = 0; i < n && s[i]; i++)` → `0 < -1` 恒假 → **循环体一次都不执行**。

所以文字从来没被画出来过（字形路径与占位路径都是）。
迷惑之处在于：不报错、不告警，而 `GetTextExtentPoint32W` 收到的是真实长度
（`wcslen`），照常登记字形请求 → 表现成"请求有、命中 0、落空 0"，
看着像外壳没接上，其实是那一行把整个循环跳过了。

### 字形管线的三条契约（改一边必须同时改另一边）

| 契约 | 原因 |
|---|---|
| 尺寸按**设备**像素 | 引擎在逻辑坐标(1000×650)排版，位图是世界层(2000×1300)。逻辑 15px 要按 30px 光栅化，否则放大 2 倍后糊成一片 |
| 位图**基线在第 px 行** | 引擎侧 `curY` 就是基线，贴图起点 = `curY - px` |
| 只存**覆盖率**，颜色由引擎上色 | 字形一律光栅化成白色（alpha 当覆盖率），贴图时用 `SetTextColor` 现算。否则所有文字同一个色，而引擎到处都是彩色文字（白正文、红"买不起"、模式色、深色描边） |

请求通道用**拉**而不是推（`platWebGlyphReq`）：引擎文案大量是运行时拼的
（阳光 123、第 4/10 波、植物名、悬停提示…），外壳硬编码字符表必然漏字。
配套 `platWebGlyphReqCount`（**不消费**的队列查询）—— 拿 `platWebGlyphReq`
去"看一眼还有没有"会把那条请求吞掉，**一个自检动作反而制造 bug**。

### 6.7.3 这一轮被静默掉的三个坑

1. **传错字节数**：`platWebGlyph` 内部 `memcpy(gw*gh*4)`，而 JS 只给了
   `gw*gh`（1 通道覆盖率）→ **越界读**，字形取到堆里别人的数据，
   画面上是**颗粒状噪声**。本机测试用手工 4 通道 buffer 所以通过了 ——
   又是"本机过、线上坏"。改为四通道同值。
2. **`const` 遮蔽**：`pumpGlyphs` 里 `let n = 0` 当计数器，循环体又写
   `const n = g.cov.length` → `n++` 变成给 const 赋值 → TypeError →
   被 catch 吞掉 → **每帧只注入 1 个字形**，字形表永远填不满。
3. **错误日志被 DEBUG 开关吞掉**：`catch` 里调的是 `log()`，
   而 `log()` 只在 `?debug=1` 时才输出 —— 异常完全不可见。
   新增 `logErr()`，**不受 DEBUG 限制**。

> 贯穿这一轮的教训：**"失败时不发声"的诊断等于没有诊断。**
> 同一件事出现了三次 —— CI 的失败上报（只在失败时贴日志，而它偏偏失败）、
> CI 的权限（贴评论要 `contents: write`，给成 `read` 就静默 403）、
> 以及这里的 `log()`。每次都表现为"我看不到原因，只能猜"。

### 新增的两条回归

| 测试 | 护的是什么 |
|---|---|
| `_test_web_printf.c`（39 条） | `wsprintfW` 的 **Windows 语义**。**必须跑在 Ubuntu 上**才有防护力：退回 `vswprintf` 委托时，只有 POSIX 平台会失败（Windows 因 MSVC 语义恰好正确而漏过） |
| `_test_web_glyph.c`（9 条） | 文字**到底有没有画到画面上**。核心断言："全图存在成块纯色" —— 字形是覆盖率贴图会形成大块同色，插画背景是连续渐变不会。用"实心假字形（覆盖率恒 255）"当输入，于是可以硬断言"像素颜色恰好等于文字颜色"，顺带验证"JS 不染色"这条契约 |

两条都在 `_regress.sh` 与 CI 里，本机（MinGW）与服务器（glibc）双通过。

### 当前状态（2026-09-21）

- 线上地址：**https://poker-pfr.site/pvz/**（旧域名 `poker-cfr.site` 无 DNS 记录）
- 服务器 `111.229.27.13`，OpenCloudOS 9，emsdk 3.1.61 编译
  （**本机装不上 emsdk**，所以编译放在服务器上做，比 GitHub Actions 稳）
- nginx 用 `location ^~ /pvz/` + `alias`，与该机上既有站点共存，未影响其 6 个路径
- 真浏览器验证：素材 **853 张**、字形 125（标题画面）/295（关卡选择页）、
  0 JS 异常、0 失败请求；导出世界层肉眼确认标题、菜单、关卡列表文字**全部正常**
- `_regress.sh`：**19 通过 / 0 失败 / 0 编译问题**
